...
**Stage definitions (info popover):** each stage carries an optional `info: StageInfo` (definition + `components[]` "Includes" checklist + `doneWhen` exit criteria) in `stages.ts`, surfaced via an ℹ️ `Popover` on the column header (`StageInfoPopover` in `AgenticBuildsBoard.tsx`). These are the single source of truth for "what this stage means" — edit `stages.ts`, not the component. **Complete** additionally carries `launchPaths[]` (project-type → production-launch step) because launch forks by build type (WP→Astro migration = DNS cutover WP Engine→Vercel; WP creative build = publish on WP Engine; WP→WP migration = cutover to new host); the popover is column-level (not per-card), so Complete lists all paths. QA's definition intentionally states form/Supabase wiring is **created or confirmed** in QA (per Paul) — flip to verify-only by editing that one `components` line.
...
**"Shown to client" marker (2026-09-10, migration `0037_build_client_shown_at`):** `tasks.clientShownAt` (nullable `timestamp`) records that a build was manually put in front of the client for beta review, **independent of stage** (the beta gate is a column divider; this is a recorded, dated fact). Toggle + date picker in `BuildDialog` (edit mode only) → flows through `UpdateBuildInput.clientShownAt` in `builds.ts` (`updateAgenticBuild`, applied only when the key is present; on = the picked date, off = null). Surfaced as an emerald 👁 "Shown {date}" badge in `BuildCard` (`AgenticBuildsBoard.tsx`). Doubles as a beta-review timing datapoint. NOTE: the column was applied to the live Central DB (`ckzqissxugofxogorrsm`) directly because deploy does not auto-migrate (`build` = `next build`, no `db:migrate` step); the `0037` migration file is idempotent (`ADD COLUMN IF NOT EXISTS`) so a later `db:migrate` is a safe no-op.
