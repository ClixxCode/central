...
import { sendSlackMessageToUser, formatTaskAssignedNotification } from '@/lib/slack';
...
export const sendAssignmentSlackNotification = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const slackConfig = await step.run('check-slack-preferences', async () => {
      ...
      if (!prefs?.notifications?.slack?.assignments) return null;
    ...
    const message = await step.run('format-message', async () => {
      return formatTaskAssignedNotification({
        ...
        actorName: data.assignerName,
        dueDate: data.taskDueDate || undefined,
      });
    });
